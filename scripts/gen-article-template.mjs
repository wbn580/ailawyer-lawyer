...
const REF = "dist/zh/articles/ailawyer-lawyer-0886/index.html"; // 参考页（已构建产物里的一篇真实文章页）
const SEG = "zh/articles";
const DEFAULT_OG = "https://ailawyer.lawyer/og.jpg"; // 站点默认 og 图（绝对 URL，可为空串）
...
head = replaceTagContent(head, /(<h1[^>]*>)([\s\S]*?)(<\/h1>)/i, "{{TITLE}}", REF_TITLE);
// <title> 标签（常带站点名后缀，如"标题 | 站点名"，只换匹配到的那一段）
head = replaceTagContent(head, /(<title[^>]*>)([\s\S]*?)(<\/title>)/i, "{{TITLE}}", REF_TITLE);
// meta name=description（两种属性顺序都认，跟 extractDesc 的探测逻辑对齐）
head = replaceTagContent(
  head, /(<meta[^>]*name=["']description["'][^>]*content=["'])([^"']*)(["'])/i, "{{DESC}}", REF_DESC,
);
head = replaceTagContent(
  head, /(<meta[^>]*content=["'])([^"']*)(["'][^>]*name=["']description["'])/i, "{{DESC}}", REF_DESC,
);
